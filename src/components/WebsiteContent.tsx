...
import { Phone, Facebook, MessageCircle, MapPin, Calculator, BookOpen, Layers, Award,
  Sparkles, ShieldCheck, CheckCircle2, ChevronRight, Zap, RefreshCw, Send, Lock, HelpCircle, Play, ExternalLink, Video, X, Menu, Eye } from 'lucide-react';
    ...
    <div className="flex-1 flex flex-col min-h-screen relative w-full font-sans">
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? `py-3 bg-[var(--background)]/90 backdrop-blur-xl border-b border-[var(--card-border)] ${
                activeTheme === 'elegant' ? 'shadow-md shadow-stone-200/40' : 'shadow-lg shadow-black/20'
              }`
            : 'py-4 bg-[var(--background)]/70 backdrop-blur-md border-b border-transparent'
        }`}
      >
        <div className="w-full max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-5 text-[13px] font-semibold">
            <a href="#about" className="hover:text-[var(--accent)] transition-colors">Giới thiệu</a>
            <a href="#products" className="hover:text-[var(--accent)] transition-colors">Quy cách in</a>
            <Link href="/thu-vien-anh" className="hover:text-[var(--accent)] transition-colors">Thư viện ảnh</Link>
            <Link href="/bao-gia" className="hover:text-[var(--accent)] transition-colors">Tính giá</Link>
            <Link href="/tra-cuu" className="hover:text-[var(--accent)] transition-colors">Tra cứu đơn</Link>
            <a href="#videos" className="hover:text-[var(--accent)] transition-colors">Video</a>
            <a href="#blog" className="hover:text-[var(--accent)] transition-colors">Cẩm nang</a>
            <a href="#contact" className="hover:text-[var(--accent)] transition-colors">Liên hệ</a>
          </nav>
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            >
              <Phone size={13} aria-hidden="true" /> 0822.968.412
            </a>
            ...
            <Link
              href="/login"
              className="p-2 border border-slate-800/60 rounded-lg text-slate-500 hover:text-[var(--accent)] hover:border-[var(--accent)]/60 transition flex items-center text-xs focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/40"
              title="Cổng quản trị"
              aria-label="Đăng nhập cổng quản trị"
            >
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-lg border border-[var(--card-border)] hover:bg-[var(--card-border)]/20 transition-all text-[var(--foreground)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/40"
              aria-label="Menu"
            >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-40 bg-[var(--background)]/98 backdrop-blur-md flex flex-col justify-center items-center gap-6 animate-fadeIn transition-all duration-300">
          {/* Close button inside mobile menu */}
          <button
            onClick={() => setMobileMenuOpen(false)}
            className="absolute top-4 right-4 p-2 rounded-lg border border-[var(--card-border)] hover:bg-[var(--card-border)]/20 transition-all text-[var(--foreground)]"
            aria-label="Đóng menu"
          >
            <X size={20} />
          </button>

          <nav className="flex flex-col items-center gap-5 text-base font-bold text-[var(--foreground)]">
            <a href="#about" onClick={() => setMobileMenuOpen(false)} className="hover:text-[var(--accent)] transition-colors py-1">Giới thiệu</a>
            <a href="#products" onClick={() => setMobileMenuOpen(false)} className="hover:text-[var(--accent)] transition-colors py-1">Quy cách in</a>
            ...
            <a href="#videos" onClick={() => setMobileMenuOpen(false)} className="hover:text-[var(--accent)] transition-colors py-1">Video</a>
            <a href="#blog" onClick={() => setMobileMenuOpen(false)} className="hover:text-[var(--accent)] transition-colors py-1">Cẩm nang</a>
            <a href="#contact" onClick={() => setMobileMenuOpen(false)} className="hover:text-[var(--accent)] transition-colors py-1">Liên hệ</a>
          </nav>

          {/* Theme switcher on mobile */}
          {!hideSwitcher && (
            <div className="flex items-center gap-1 bg-black/10 border border-[var(--card-border)] rounded-lg p-1 mt-4">
              <button
                ...
                className={`px-3 py-1.5 rounded text-xs font-bold tracking-wide transition-all ${
                  activeTheme === 'tech' ? 'bg-sky-500 text-slate-950 shadow-md' : 'text-[var(--text-muted)] hover:text-[var(--foreground)]'
                }`}
              >
                M1
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded text-xs font-bold tracking-wide transition-all ${
                  activeTheme === 'creative' ? 'bg-pink-500 text-white shadow-md' : 'text-[var(--text-muted)] hover:text-[var(--foreground)]'
                }`}
              >
                M2
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded text-xs font-bold tracking-wide transition-all ${
                  activeTheme === 'elegant' ? 'bg-amber-600 text-white shadow-md' : 'text-[var(--text-muted)] hover:text-[var(--foreground)]'
                }`}
              >
                M3
              </button>
