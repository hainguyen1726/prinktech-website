    ...
    <div className="admin-panel min-h-screen bg-[#f8fafc] text-slate-900 transition-colors duration-300">
      ...
      <nav className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur-xl shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-slate-900 font-bold text-sm">Quản lý Khách hàng</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              ...
              className="px-3 py-1.5 rounded-xl text-xs font-bold bg-amber-700 text-white shadow-xs transition-all"
            >
              👥 Khách hàng
            </Link>
            <Link
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900">Quản Lý Khách Hàng Prink Tech</h1>
            <p className="text-slate-500 text-sm mt-1">
              Danh sách khách hàng đặt in tem trực tiếp từ Website PrinK Tech (tách biệt riêng với tài khoản đại lý xưởng).
            </p>
          </div>
          <button
            onClick={() => alert('Tính năng thêm mới khách hàng trực tiếp đang được cập nhật')}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-700 hover:bg-amber-800 text-white font-bold text-sm shadow-sm transition-all cursor-pointer self-start md:self-auto"
          >
        ...
        {/* Filter Bar */}
        <div className="rounded-2xl border border-slate-200 bg-white p-4 mb-6 flex flex-col sm:flex-row gap-3 shadow-xs">
          ...
          <button
            ...
            className="h-10 px-5 rounded-xl bg-slate-800 hover:bg-slate-900 text-white font-bold text-sm transition-all cursor-pointer"
          >
            Tìm kiếm
          </button>
        </div>
        ...
        {loading && (
          <div className="text-center py-12 text-slate-500 flex items-center justify-center gap-2">
            <RefreshCw className="animate-spin text-amber-700" size={20} /> Đang tải dữ liệu khách hàng...
          </div>
        )}
        ...
        {!loading && customers.length === 0 && (
          <div className="text-center py-16 border border-dashed border-slate-200 rounded-2xl bg-white text-slate-400 font-medium">
            Chưa tìm thấy khách hàng nào trong hệ thống PrinK Tech Website.
          </div>
        )}

        {/* DATA TABLE LAYOUT FOR CUSTOMER LIST */}
        {!loading && customers.length > 0 && (
                  ...
                  {customers.map(customer => {
                    ...
                    return (
                      <tr key={customer.id} className="group hover:bg-slate-50/80 transition-colors">
                        {/* Expanded details container inside sub-row */}
                                      ...
                                      {isOrderExpanded && (
                                        <div className="border-t border-slate-200 bg-white">
                                          ...
                                          <div className="flex border-b border-slate-200 bg-slate-50 text-xs font-bold">
                                            ...
                                            >
                                              🏷️ Mẫu Tem / Sản Phẩm ({order.items ? order.items.length : 0})
                                            </button>
                                            <button
                                            ...
                                            >
                                              📜 Lịch Sử Thay Đổi ({order.history ? order.history.length : 0})
                                            </button>
                                          </div>
                                                ...
                                                <div className="space-y-2 border-l-2 border-amber-600/30 pl-3">
                                                  {order.history.map(hist => (
                                                    <div key={hist.id} className="relative pb-2">
                                                      <div className="font-semibold text-slate-800">{hist.description}</div>
