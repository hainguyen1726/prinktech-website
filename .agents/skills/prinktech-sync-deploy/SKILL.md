...
1. **Đồng bộ code master tại Dev Workspace:**
   ```bash
...
3. **Thực hiện chỉnh sửa và build thử nghiệm:**
   Sau khi chỉnh sửa xong, chạy script `check-and-fix.sh` trong Dev Workspace để đảm bảo không lỗi.
   *Lưu ý về deploy Production*: Hermes **chỉ được phép** nhảy sang thư mục Production `/srv/website-prinktech` và chạy `bash deploy-vps-git.sh` khi có sự chỉ định hoặc đồng ý trực tiếp của người dùng.
