...
if [ -n "$FOUND_CADDYFILE" ]; then
    ...
    # Thực hiện swap cấu hình proxy trong Caddyfile bằng Python regex linh hoạt & Inode-safe
    python3 -c "
import sys, re
...
" "$FOUND_CADDYFILE" "$TARGET_CONTAINER:3000"
    ...
    if docker ps | grep -q "n8n-caddy-1"; then
        ...
        success "Caddy reload thành công! Traffic mới trỏ về $TARGET_CONTAINER:3000 (cổng host: $TARGET_PORT)."
    else
